...
function loadSubsystemList(paramJson) {
  $ .getJSON("<c:url value='/spring/am/subsystemPage'/>", paramJson, function(page) {
	$("#list-items").html("");
...
function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"spring/am/subsystem/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
...
function save() {
  var saveUrl = "<c:url value='/spring/am/subsystem'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "spring/am/subsystem/" + mSelUuids;
  }
