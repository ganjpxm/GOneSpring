...
function loadUserRoleList(paramJson) {
  $ .getJSON("<c:url value='/am/userRolePage'/>", paramJson, function(page) {
	$("#list-items").html("");
...
function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"am/userRole/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
...
function save() {
  var saveUrl = "<c:url value='/am/userRole'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "am/userRole/" + mSelUuids;
  }
...
function del() {
  var urlStr = "<c:url value='/am/userRole/delete'/>";
