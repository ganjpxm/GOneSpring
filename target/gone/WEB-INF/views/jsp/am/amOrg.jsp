...
<body>
<div id="orgs" class="jp-box">
  <div class="jp-title-bar">
    <div class="jp-title-bar-text">Org</div>
...
<div id="org" class="jp-box jp-hide">
  <div class="jp-title-bar" style="text-align:center;position:fixed;">
    <div id="org-title" style="padding-top:10px;font-size:18px;"></div>
...
function search() {
  ...
  loadOrgList(mParamJson);
}

function loadOrgList(paramJson) {
  $ .getJSON("<c:url value='/am/orgPage'/>", paramJson, function(page) {
	$("#list-items").html("");
	var data = page.result;
	$ .each(data, function(index, map) {
	  var uuid = "";
	  var listTds = [];
	  listTds.push("<td><input name='uuid' type='checkbox' class='jp-check-box' value='" + map['orgId'] + "'/></td>");
...
function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"am/org/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
...
function save() {
  var saveUrl = "<c:url value='/am/org'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "am/org/" + mSelUuids;
  }
  $("#org-form").ajaxSubmit({
    dataType:  'json',
    url: saveUrl,
...
function del() {
  var urlStr = "<c:url value='/am/org/delete'/>";
