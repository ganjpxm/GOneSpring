/**
 * $Id: CmFileDaoImpl.java, v1.0 2014/12/17 11:31:49 GanJianping Exp $
 *
 ...
 */
@Repository
public class CmFileDaoImpl extends AbstractHibernateDao<CmFile> implements CmFileDao {

    public CmFileDaoImpl() {
        super();

        setClazz(CmFile.class);
    }
	...
	public Page<CmFile> getCmFilePage(final String search, final String startDate, final String endDate, final String dataStates,
		...
		String hql = "select a from CmFile a where 1=1 ";
	...
	public Page<CmFile> getCmFilePage(String keywords, String field, final String lang, final String startDate, final String endDate, final int pageNo,
		...
		String hql = "from CmFile where dataStatus=? ";
		List<Object> paramList = new ArrayList<Object>();
			...
			for (int i=0; i<andKeywordArr.length; i++) {
				...
				for (int j=0; j<orKeywordArr.length; j++) {
					...
					if ("name".equalsIgnoreCase(field)) {
						...
						hql +=" fileName like ? or tags like ? ";
						paramList.add("%" + orKeywordArr[j] + "%");
						paramList.add("%" + orKeywordArr[j] + "%");
					}
				}
