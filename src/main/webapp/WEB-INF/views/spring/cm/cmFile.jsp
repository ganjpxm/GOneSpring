...
<body>
<div id="files" class="jp-box">
  <div class="jp-title-bar">
    <div class="jp-title-bar-text">File</div>
...
<div id="file" class="jp-box jp-hide">
  <div class="jp-title-bar" style="text-align:center;position:fixed;">
    <div id="file-title" style="padding-top:10px;font-size:18px;"></div>
...
function search() {
  ...
  loadFileList(mParamJson);
}

function loadFileList(paramJson) {
  $ .getJSON("<c:url value='/spring/cm/filePage'/>", paramJson, function(page) {
	$("#list-items").html("");
	var data = page.result;
	$ .each(data, function(index, map) {
	  var uuid = "";
	  var listTds = [];
	  listTds.push("<td><input name='uuid' type='checkbox' class='jp-check-box' value='" + map['fileId'] + "'/></td>");
...
function add() {
  ...
  $("#files").hide();
  $("#file").show();
  $("#file-title").html("Add File");
  mIsAdd = true;
}

function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"spring/cm/file/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
...
function save() {
  var saveUrl = "<c:url value='/spring/cm/file'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "spring/cm/file/" + mSelUuids;
  }
...
function del() {
  var urlStr = "<c:url value='/spring/cm/file/delete'/>";
  $ .ajax({type:"POST", url:urlStr, data: {fileIds:mSelUuids}, async:true, dataType:'json', 
	success : function(data) {
	  if (data.result=="success") {
		loadFileList(mParamJson);
		$("#files").show();
