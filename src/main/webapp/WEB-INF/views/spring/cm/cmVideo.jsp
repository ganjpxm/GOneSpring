...
<body>
<div id="videos" class="jp-box">
  <div class="jp-title-bar">
    <div class="jp-title-bar-text">Video</div>
...
<div id="video" class="jp-box jp-hide">
  <div class="jp-title-bar" style="text-align:center;position:fixed;">
    <div id="video-title" style="padding-top:10px;font-size:18px;"></div>
...
function search() {
  ...
  loadVideoList(mParamJson);
}

function loadVideoList(paramJson) {
  $ .getJSON("<c:url value='/spring/cm/videoPage'/>", paramJson, function(page) {
	$("#list-items").html("");
	var data = page.result;
	$ .each(data, function(index, map) {
	  var uuid = "";
	  var listTds = [];
	  listTds.push("<td><input name='uuid' type='checkbox' class='jp-check-box' value='" + map['videoId'] + "'/></td>");
...
function add() {
  ...
  $("#videos").hide();
  $("#video").show();
  $("#video-title").html("Add Video");
  mIsAdd = true;
}

function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"spring/cm/video/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
...
function save() {
  var saveUrl = "<c:url value='/spring/cm/video'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "spring/cm/video/" + mSelUuids;
  }
...
function del() {
  var urlStr = "<c:url value='/spring/cm/video/delete'/>";
  $ .ajax({type:"POST", url:urlStr, data: {videoIds:mSelUuids}, async:true, dataType:'json', 
	success : function(data) {
	  if (data.result=="success") {
		loadVideoList(mParamJson);
		$("#videos").show();
