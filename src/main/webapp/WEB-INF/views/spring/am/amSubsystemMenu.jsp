...
<body>
<div id="subsystemMenus" class="jp-box">
  <div class="jp-title-bar">
    <div class="jp-title-bar-text">SubsystemMenu</div>
...
<div id="subsystemMenu" class="jp-box jp-hide">
  <div class="jp-title-bar" style="text-align:center;position:fixed;">
    <div id="subsystemMenu-title" style="padding-top:10px;font-size:18px;"></div>
  ...
  <form id="subsystemMenu-form" method="POST">
...
function search() {
  ...
  loadSubsystemMenuList(mParamJson);
}

function loadSubsystemMenuList(paramJson) {
  $ .getJSON("<c:url value='/spring/am/subsystemMenuPage'/>", paramJson, function(page) {
	$("#list-items").html("");
	var data = page.result;
	$ .each(data, function(index, map) {
	  var uuid = "";
	  var listTds = [];
	  listTds.push("<td><input name='uuid' type='checkbox' class='jp-check-box' value='" + map['subsystemMenuId'] + "'/></td>");
...
function add() {
  ...
  $("#subsystemMenus").hide();
  $("#subsystemMenu").show();
  $("#subsystemMenu-title").html("Add SubsystemMenu");
  mIsAdd = true;
}

function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"spring/am/subsystemMenu/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
	  ...
	  $("#subsystemMenus").hide();
	  $("#subsystemMenu").show();
	  $("#subsystemMenu-title").html("Edit SubsystemMenu");
  } else {
...
function save() {
  var saveUrl = "<c:url value='/spring/am/subsystemMenu'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "spring/am/subsystemMenu/" + mSelUuids;
  }
  $("#subsystemMenu-form").ajaxSubmit({
    dataType:  'json',
    url: saveUrl,
...
function del() {
  var urlStr = "<c:url value='/spring/am/subsystemMenu/delete'/>";
  $ .ajax({type:"POST", url:urlStr, data: {subsystemMenuIds:mSelUuids}, async:true, dataType:'json', 
	success : function(data) {
	  if (data.result=="success") {
		loadSubsystemMenuList(mParamJson);
		$("#subsystemMenus").show();
