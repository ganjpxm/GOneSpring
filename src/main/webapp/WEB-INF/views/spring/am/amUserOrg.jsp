...
<body>
<div id="userOrgs" class="jp-box">
  <div class="jp-title-bar">
    <div class="jp-title-bar-text">UserOrg</div>
...
<div id="userOrg" class="jp-box jp-hide">
  <div class="jp-title-bar" style="text-align:center;position:fixed;">
    <div id="userOrg-title" style="padding-top:10px;font-size:18px;"></div>
  ...
  <form id="userOrg-form" method="POST">
...
function search() {
  ...
  loadUserOrgList(mParamJson);
}

function loadUserOrgList(paramJson) {
  $ .getJSON("<c:url value='/spring/am/userOrgPage'/>", paramJson, function(page) {
	$("#list-items").html("");
	var data = page.result;
	$ .each(data, function(index, map) {
	  var uuid = "";
	  var listTds = [];
	  listTds.push("<td><input name='uuid' type='checkbox' class='jp-check-box' value='" + map['userOrgId'] + "'/></td>");
...
function add() {
  ...
  $("#userOrgs").hide();
  $("#userOrg").show();
  $("#userOrg-title").html("Add UserOrg");
  mIsAdd = true;
}

function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"spring/am/userOrg/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
	  ...
	  $("#userOrgs").hide();
	  $("#userOrg").show();
	  $("#userOrg-title").html("Edit UserOrg");
  } else {
...
function save() {
  var saveUrl = "<c:url value='/spring/am/userOrg'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "spring/am/userOrg/" + mSelUuids;
  }
  $("#userOrg-form").ajaxSubmit({
    dataType:  'json',
    url: saveUrl,
...
function del() {
  var urlStr = "<c:url value='/spring/am/userOrg/delete'/>";
  $ .ajax({type:"POST", url:urlStr, data: {userOrgIds:mSelUuids}, async:true, dataType:'json', 
	success : function(data) {
	  if (data.result=="success") {
		loadUserOrgList(mParamJson);
		$("#userOrgs").show();
