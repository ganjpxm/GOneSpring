...
<body>
<div id="roleMenus" class="jp-box">
  <div class="jp-title-bar">
    <div class="jp-title-bar-text">RoleMenu</div>
...
<div id="roleMenu" class="jp-box jp-hide">
  <div class="jp-title-bar" style="text-align:center;position:fixed;">
    <div id="roleMenu-title" style="padding-top:10px;font-size:18px;"></div>
  ...
  <form id="roleMenu-form" method="POST">
...
function search() {
  ...
  loadRoleMenuList(mParamJson);
}

function loadRoleMenuList(paramJson) {
  $ .getJSON("<c:url value='/spring/am/roleMenuPage'/>", paramJson, function(page) {
	$("#list-items").html("");
	var data = page.result;
	$ .each(data, function(index, map) {
	  var uuid = "";
	  var listTds = [];
	  listTds.push("<td><input name='uuid' type='checkbox' class='jp-check-box' value='" + map['roleMenuId'] + "'/></td>");
...
function add() {
  ...
  $("#roleMenus").hide();
  $("#roleMenu").show();
  $("#roleMenu-title").html("Add RoleMenu");
  mIsAdd = true;
}

function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"spring/am/roleMenu/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
	  ...
	  $("#roleMenus").hide();
	  $("#roleMenu").show();
	  $("#roleMenu-title").html("Edit RoleMenu");
  } else {
...
function save() {
  var saveUrl = "<c:url value='/spring/am/roleMenu'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "spring/am/roleMenu/" + mSelUuids;
  }
  $("#roleMenu-form").ajaxSubmit({
    dataType:  'json',
    url: saveUrl,
...
function del() {
  var urlStr = "<c:url value='/spring/am/roleMenu/delete'/>";
  $ .ajax({type:"POST", url:urlStr, data: {roleMenuIds:mSelUuids}, async:true, dataType:'json', 
	success : function(data) {
	  if (data.result=="success") {
		loadRoleMenuList(mParamJson);
		$("#roleMenus").show();
