...
<body>
<div id="roleSubsystems" class="jp-box">
  <div class="jp-title-bar">
    <div class="jp-title-bar-text">RoleSubsystem</div>
...
<div id="roleSubsystem" class="jp-box jp-hide">
  <div class="jp-title-bar" style="text-align:center;position:fixed;">
    <div id="roleSubsystem-title" style="padding-top:10px;font-size:18px;"></div>
  ...
  <form id="roleSubsystem-form" method="POST">
...
function search() {
  ...
  loadRoleSubsystemList(mParamJson);
}

function loadRoleSubsystemList(paramJson) {
  $ .getJSON("<c:url value='/spring/am/roleSubsystemPage'/>", paramJson, function(page) {
	$("#list-items").html("");
	var data = page.result;
	$ .each(data, function(index, map) {
	  var uuid = "";
	  var listTds = [];
	  listTds.push("<td><input name='uuid' type='checkbox' class='jp-check-box' value='" + map['roleSubsystemId'] + "'/></td>");
...
function add() {
  ...
  $("#roleSubsystems").hide();
  $("#roleSubsystem").show();
  $("#roleSubsystem-title").html("Add RoleSubsystem");
  mIsAdd = true;
}

function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"spring/am/roleSubsystem/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
	  ...
	  $("#roleSubsystems").hide();
	  $("#roleSubsystem").show();
	  $("#roleSubsystem-title").html("Edit RoleSubsystem");
  } else {
...
function save() {
  var saveUrl = "<c:url value='/spring/am/roleSubsystem'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "spring/am/roleSubsystem/" + mSelUuids;
  }
  $("#roleSubsystem-form").ajaxSubmit({
    dataType:  'json',
    url: saveUrl,
...
function del() {
  var urlStr = "<c:url value='/spring/am/roleSubsystem/delete'/>";
  $ .ajax({type:"POST", url:urlStr, data: {roleSubsystemIds:mSelUuids}, async:true, dataType:'json', 
	success : function(data) {
	  if (data.result=="success") {
		loadRoleSubsystemList(mParamJson);
		$("#roleSubsystems").show();
