...
<body>
<div id="configs" class="jp-box">
  <div class="jp-title-bar">
    <div class="jp-title-bar-text">Config</div>
...
<div id="config" class="jp-box jp-hide">
  <div class="jp-title-bar" style="text-align:center;position:fixed;">
    <div id="config-title" style="padding-top:10px;font-size:18px;"></div>
...
function search() {
  ...
  loadConfigList(mParamJson);
}

function loadConfigList(paramJson) {
  $ .getJSON("<c:url value='/bm/configPage'/>", paramJson, function(page) {
	$("#list-items").html("");
	var data = page.result;
	$ .each(data, function(index, map) {
	  var uuid = "";
	  var listTds = [];
	  listTds.push("<td><input name='uuid' type='checkbox' class='jp-check-box' value='" + map['configId'] + "'/></td>");
...
function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"bm/config/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
...
function save() {
  var saveUrl = "<c:url value='/bm/config'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "bm/config/" + mSelUuids;
  }
...
function del() {
  var urlStr = "<c:url value='/bm/config/delete'/>";
