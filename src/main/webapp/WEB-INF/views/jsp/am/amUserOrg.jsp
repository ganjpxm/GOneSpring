...
function loadUserOrgList(paramJson) {
  $ .getJSON("<c:url value='/am/userOrgPage'/>", paramJson, function(page) {
	$("#list-items").html("");
...
function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"am/userOrg/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
...
function save() {
  var saveUrl = "<c:url value='/am/userOrg'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "am/userOrg/" + mSelUuids;
  }
...
function del() {
  var urlStr = "<c:url value='/am/userOrg/delete'/>";
