...
<body>
<div id="userRoles" class="jp-box">
  <div class="jp-title-bar">
    <div class="jp-title-bar-text">UserRole</div>
...
<div id="userRole" class="jp-box jp-hide">
  <div class="jp-title-bar" style="text-align:center;position:fixed;">
    <div id="userRole-title" style="padding-top:10px;font-size:18px;"></div>
  ...
  <form id="userRole-form" method="POST">
...
function search() {
  ...
  loadUserRoleList(mParamJson);
}

function loadUserRoleList(paramJson) {
  $ .getJSON("<c:url value='/spring/am/userRolePage'/>", paramJson, function(page) {
	$("#list-items").html("");
	var data = page.result;
	$ .each(data, function(index, map) {
	  var uuid = "";
	  var listTds = [];
	  listTds.push("<td><input name='uuid' type='checkbox' class='jp-check-box' value='" + map['userRoleId'] + "'/></td>");
...
function add() {
  ...
  $("#userRoles").hide();
  $("#userRole").show();
  $("#userRole-title").html("Add UserRole");
  mIsAdd = true;
}

function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"spring/am/userRole/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
	  ...
	  $("#userRoles").hide();
	  $("#userRole").show();
	  $("#userRole-title").html("Edit UserRole");
  } else {
...
function save() {
  var saveUrl = "<c:url value='/spring/am/userRole'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "spring/am/userRole/" + mSelUuids;
  }
  $("#userRole-form").ajaxSubmit({
    dataType:  'json',
    url: saveUrl,
...
function del() {
  var urlStr = "<c:url value='/spring/am/userRole/delete'/>";
  $ .ajax({type:"POST", url:urlStr, data: {userRoleIds:mSelUuids}, async:true, dataType:'json', 
	success : function(data) {
	  if (data.result=="success") {
		loadUserRoleList(mParamJson);
		$("#userRoles").show();
