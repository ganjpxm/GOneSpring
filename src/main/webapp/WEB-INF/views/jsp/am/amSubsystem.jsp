...
<body>
<div id="subsystems" class="jp-box">
  <div class="jp-title-bar">
    <div class="jp-title-bar-text">Subsystem</div>
...
<div id="subsystem" class="jp-box jp-hide">
  <div class="jp-title-bar" style="text-align:center;position:fixed;">
    <div id="subsystem-title" style="padding-top:10px;font-size:18px;"></div>
...
function search() {
  ...
  loadSubsystemList(mParamJson);
}

function loadSubsystemList(paramJson) {
  $ .getJSON("<c:url value='/am/subsystemPage'/>", paramJson, function(page) {
	$("#list-items").html("");
	var data = page.result;
	$ .each(data, function(index, map) {
	  var uuid = "";
	  var listTds = [];
	  listTds.push("<td><input name='uuid' type='checkbox' class='jp-check-box' value='" + map['subsystemId'] + "'/></td>");
...
function edit() {
  if (!jp.isEmpty(mSelUuids) && mSelUuids.length==32) {
	  $ .getJSON(mRootUrl+"am/subsystem/" + mSelUuids, function(data) {
		  var formFields = [];
		  $ .each(mFieldNames, function(index, fieldName) {
...
function save() {
  var saveUrl = "<c:url value='/am/subsystem'/>";
  if (mIsAdd == false) {
	  saveUrl = mRootUrl + "am/subsystem/" + mSelUuids;
  }
...
function del() {
  var urlStr = "<c:url value='/am/subsystem/delete'/>";
